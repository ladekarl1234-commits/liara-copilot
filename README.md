...
**Citation depth, measured — not claimed.** Only **36.5%** of chunks carry an authored `<Section id=…>` anchor (`anchorCoverage` in the committed eval artifact), so "deep-anchor citations" was never true of most of the corpus (`EP-PRD-06` / `EP-RET-09`). Chunks without one now cite a `#:~:text=` fragment that scrolls to and highlights the sentence the answer came from. Over the built index that is **36.5% anchored + 57.4% text-fragment = 93.9% deep-linked**; the remaining **6.1%** have no usable prose line and land at the top of the page. Both figures are re-measured from the built index on every CI run (`tests/docs-numbers.test.ts`), not transcribed once.
